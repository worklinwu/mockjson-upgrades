...
var _mocked = [];
var mockJSON = function (_tpl, _data) { // _tpl 为正则表达式
    for (var i = 0; i < _mocked.length; i++) {
        var mock = _mocked[i];
        if (mock.tpl.toString() == _tpl.toString()) {
            _mocked.splice(i, 1);
            break;
        }
    }
...
/**
 * 新扩展的方法, 解析模板, 生成数据
 * @param _tpl   预定好的模板, 如果预先定义了要返回的数据, 则直接返回, 忽略url带的data参数
 * @param _data   json格式的模板, 规则参考 http://experiments.mennovanslooten.nl/2010/mockjson/
 * @returns {*}  返回生成好的json数据
 */
mockJSON.parse = function (_tpl, _data) {
    for (var i = 0; i < _mocked.length; i++) {
        var mock = _mocked[i];
        if (mock.tpl.test(_tpl)) {
            return mockJSON.generateFromTemplate(mock.data);
        }
    }
    return mockJSON.generateFromTemplate(_data);
};
