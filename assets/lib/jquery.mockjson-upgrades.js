/*
 * file name : mockjson 加强版
 * author : linwu (http://www.linwu.name/)
 * desc : 在原有的功能基础上添加一些新的使用方法和功能
 * e.g.
 *      $.ajax({
            url: "foo.php",
            type: "get",
    ...
    原 mockJson Github地址 : https://github.com/mennovanslooten/mockJSON/blob/master/js/jquery.mockjson.js
 */
(function ($) {
    ...
    $.mockJSON = function (request, template) {
        for (var i = 0; i < _mocked.length; i++) {
            var mock = _mocked[i];
            if (mock.tpl.toString() == request.toString()) {
                _mocked.splice(i, 1);
                break;
            }
        }
    ...
    $.ajax = function (options) {
        // 输出的对象
        var response;
        // 判断是否有mock参数, 如果有, 无视url规则, 直接返回mock的data参数
        if (options.mock) {
            response = $.mockJSON.generateFromTemplate(options.mock.data);
            // 如果有定义 beforeSend, 执行 beforeSend 函数, 但是没有参数.
            options.beforeSend && options.beforeSend();
            setTimeout(function () {
                // 执行 success 函数, 把 解析后的数据返回.
                options.success(response);
                // 如果有定义 beforeSend, 执行 beforeSend 函数, 但是没有参数.
                options.complete && options.complete();
                return $;
            }, options.mock.delay ? options.mock.delay : 0); // 如果有 delay 参数, 则延迟执行. (模拟网络延迟)
        } else {
            // 去掉了 dataType==json 的判断
            for (var i = 0; i < _mocked.length; i++) {
                var mock = _mocked[i];
                if (mock.tpl.test(options.url)) {
                    // beforeSend
                    options.beforeSend && options.beforeSend();
                    options.success($.mockJSON.generateFromTemplate(mock.data));
                    // complete
                    options.complete && options.complete();
                    return $;
                }
